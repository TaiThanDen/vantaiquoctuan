...
export default function TruckTypesPage() {
  ...
  const [searchResults, setSearchResults] = useState<TruckTypes[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    TruckTypesClient.getAll(page, limit)
      .then(setTruckTypes)
  ...
  const displayTruckTypes = keyword.trim() ? searchResults : truckTypes;
